...
def _semantic_issues(doc: dict[str, Any]) -> list[Issue]:
    ...
    for index, dive in enumerate(collections["dives"]):
        ...
        _check_reference(dive, "center_uuid", known["centers"], "centers", here, issues)
    ...
    for index, trip in enumerate(collections["trips"]):
        ...
        for part_index, part in enumerate(trip.get("parts") or []):
            ...
            # collection: §5.3 resolves it where §6.9a says, in `centers`.
            _check_reference(part, "accommodation_uuid", known["centers"], "centers", part_path, issues)
    ...
    for index, record in enumerate(collections["gear_service_records"]):
        ...
        _check_reference(record, "center_uuid", known["centers"], "centers", here, issues)
    ...
    for index, course in enumerate(collections["courses"]):
        if _present(course, "starts_on") and _present(course, "ends_on"):
            try:
                ...
                pass
        _check_reference(course, "center_uuid", known["centers"], "centers", f"courses/{index}", issues)
    ...
    for index, certification in enumerate(collections["certifications"]):
        ...
        _check_reference(certification, "center_uuid", known["centers"], "centers", here, issues)
