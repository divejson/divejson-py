...
| `accommodation-dangling-reference.divejson` | a trip part's `accommodation_uuid` resolves to no contact — the embedded host §3's rule 1 now reaches, where `dangling-reference` is a record's | §5.3, §6.9a |
| `contact-roles-repeated.divejson` | a contact's `roles` carries `school` twice | §6.18 |
| `address-without-country.divejson` | a contact's `address` with no `country` | §6.19 |
...
four hundred waypoints. `uddf/contacts.uddf` is reduced from nothing: no export in hand
...
eye. `write/uddf/contacts.divejson` is the one, written for the contacts rules no other
document needed — a shop, a liveaboard, a contact two parts share that carries nothing but its
name, and two contacts sharing one.
...
Every input but `contacts.divejson` is a document this tree already carries, and the pairs
