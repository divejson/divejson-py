...
eye. `write/uddf/centers.divejson` is the one, written for the centers rules no other
document needed — a shop, a liveaboard, a center two parts share that carries nothing but its
name, and two centers sharing one.
...
| `centers.divejson` | hand-built | The writing half of §6.18 where `technical-dive` cannot reach it, five centers. Blue Hole Divers, carrying every member but `extensions` and referenced from all five hosts, as a `<divebase>`. Reefside, a shop and nothing else, as a `<business><shop>`, the one slot that says its role back and the only center here whose `roles` is not reported. Northern Star Liveaboards as a `<divebase>`, and as the copy on the part spent aboard, typed `boat`. Grandma's house, a name and a role, which two parts stay at: a name-only base and two copies of it, which a reader folds back into one center where one that did not fold would return three. And a guest house also called Reefside, a name and a role, which the last part stays at: its name being the shop's, that part gets no copy and its accommodation is reported, and with no copy naming it and no dive linking it, its name-only base is one a reader skips, so it is reported too. |
