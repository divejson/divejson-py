...
`uddf/` is the third corpus and is for *converters* rather than validators: each input is
paired with the document a correct reader produces from it.
...
another language can be run against. The Python converter in this repository is the first
and deliberately not the last.
...
**Every input here is hand-built**, reduced from a real export of that writer rather than
