...
| `centers`, and every reference to one | Subsurface has no field for a dive center, a shop or a place stayed: the people a dive records are a buddy and a divemaster, and its one place is the site. Its UDDF export writes one placeholder `<divebase>` per file for want of one ([`uddf-mapping.md`](uddf-mapping.md), *Centers*). |
